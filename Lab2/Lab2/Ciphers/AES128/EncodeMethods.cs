...
namespace Lab2
{
    ...
    {
        // метод шифрования
        readonly byte[] sBox = new byte[256] {
            ...
            0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16 }; //F // таблица для замен байтов метода SubBytes
        readonly byte[] mul2 = {
            ...
            0xfb,0xf9,0xff,0xfd,0xf3,0xf1,0xf7,0xf5,0xeb,0xe9,0xef,0xed,0xe3,0xe1,0xe7,0xe5 }; // таблица умножения любого числа на 2 в поле Галуа
        readonly byte[] mul3 = {
        ...
        }; // таблица умножения любого числа на 3 в поле Галуа

        public string Encode(string textToEncode)
        {
            string encodedText;
            byte[] bytesToCipher = FromStringToBytes(textToEncode);
            // шифрование
            for (int i = 0; i < bytesToCipher.Length; i += blockSize)
            {
                byte[] state = new byte[blockSize];
                Array.Copy(bytesToCipher, i, state, 0, blockSize);
                ...
                Array.Copy(state, 0, bytesToCipher, i, blockSize);
            }
            Console.WriteLine();
            encodedText = Convert.ToBase64String(bytesToCipher);
            return encodedText;
        }
